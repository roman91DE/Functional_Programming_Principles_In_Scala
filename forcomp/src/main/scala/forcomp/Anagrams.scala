...
object Anagrams extends AnagramsInterface:
  ...
  def wordOccurrences(w: Word): Occurrences = {
    ...
    val grouped = chars.groupMapReduce(identity(_))(x => 1)(_ + _)
    val result = grouped.toList.sortBy(p => p._1)
    result
  }
  ...
  lazy val dictionaryByOccurrences: Map[Occurrences, List[Word]] = {
    val li: List[(Occurrences, Word)] =
      dictionary.map(w => (wordOccurrences(w), w))
    val d: Map[Occurrences, List[Word]] =
      li.groupMapReduce(_._1)(x => List(x._2))(_ ::: _)
    d
  }

  /** Returns all the anagrams of a given word. */
  def wordAnagrams(word: Word): List[Word] = {
    val pattern = wordOccurrences(word)
    val matches = dictionaryByOccurrences.get(pattern)
